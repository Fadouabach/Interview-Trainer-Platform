...
function AuthenticatedApp() {
  ...
  const finishSession = async (sessionResults) => {
    ...
    try {
      ...
      // Calculate a dummy score for now (e.g. 80 + rand(20))
      const dummyScore = Math.floor(Math.random() * 21) + 80;

      await axios.post('http://localhost:5000/api/interviews', {
        userId: user.id,
        category,
        duration: 300, // Placeholder 5 mins
        score: dummyScore,
        answers: sessionResults.map(r => ({
